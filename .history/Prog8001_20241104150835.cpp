...
// dir = 0 for up, 1 for right, 2 for down, 3 for left
int moveRat(int ratx, int raty, int len, int dir, int moves)
{
	...
	dir = (dir + 1) % 4;
	return moveRat(ratx, raty, limit, dir, ++moves);
}

void findFood(double len)
{
    int moves = moveRat(0, 0, len, 0, 0);
    cout << "The Rat gets the food in " << moves << " Moves.";
}
